...
function applyDesign(input: ApplyDesignInput): void {
    ...
    if (input.customVariables && Object.keys(input.customVariables).length > 0) {
        ...
        console.log(`✅ Custom theme injected into ${CSS_FILE}`);
        console.log(`   Set data-theme="custom" to use it.`);
    }

    if (input.selectedTheme !== undefined) {
        // Create/update a simple script that sets the theme on load
...
`;
        ...
        console.log(`✅ Theme '${input.selectedTheme || "default"}' will be applied on page load.`);
        console.log(`   Script created: ${themeInitPath}`);
    }
}

function main() {
    console.log("🎨 Applying design changes...\n");

    try {
        const input = readInputFile();

        if (!input.selectedTheme && !input.customVariables) {
            console.error("❌ Error: Input must contain 'selectedTheme' or 'customVariables'");
